<template>
  <div>
    ...
    <v-nav-menu-item v-if="currentUser.roles.includes('[ICE] Member') || currentUser.roles.includes('[ICE] Applicant')"
                     to="/hub" icon="HomeIcon">
    ...
    <v-nav-menu-item
      v-if="currentUser.roles.some(role => ['[ICE] Recruiter', '[ICE] Owner', '[ICE] Admin', '[ICE] Webmaster'].includes(role))"
      to="/admin/applications"
      icon="FileIcon">
      <span v-show="!verticalNavMenuItemsMin" class="truncate">Applications</span>
    </v-nav-menu-item>
    <v-nav-menu-item
      v-if="currentUser.roles.some(role =>['[ICE] Owner', '[ICE] Admin', '[17th] Alpha Company HQ', '[17th] Officer', '[ICE] Webmaster', '[CGS] Owner'].includes(role))"
      to="/admin/channels"
      icon="HashIcon">
    ...
    <v-nav-menu-item
      v-if="currentUser.roles.some(role =>['[ICE] Admin', '[ICE] Owner', '[ICE] Webmaster'].includes(role))"
      to="/admin/iceberg/disciplinary-action"
      icon="ArchiveIcon">
      <span v-show="!verticalNavMenuItemsMin" class="truncate">View Disciplinary Action Forms</span>
    </v-nav-menu-item>

    <v-nav-menu-item
      v-if="channels && currentUser.roles.includes('[ICE] Member') && channel.division.toLowerCase() === 'iceberg'"
