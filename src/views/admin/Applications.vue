        ...
        <div v-for="app in applications" v-bind:key="app.id" class="vx-col w-full lg:w-1/4 sm:w-1/2 mb-base">
          <vx-card :title="user(app.userID).username" :subtitle="'Division: ' + applicationDivisionDisplay(app.division)" class="p-2">
            <div class="text-center">
              ...
              <vs-chip v-else-if="application(app.id).status.toLowerCase() === 'processed'" color="warning">{{ application(app.id).status }}</vs-chip>
...
export default {
  ...
  computed: {
    ...mapGetters(['applications', 'currentUser', 'user', 'users', 'application']),
  },
  async created() {
    await Promise.all([
