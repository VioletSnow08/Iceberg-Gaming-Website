<template>
  <div>
    ...
    <div v-else-if="application($route.params.applicationID)">
      {{ this.$vs.loading.close() }}
      <div class="text-center">
        <h2>Now Viewing Your Application</h2>
        <p>Current Status: {{ application($route.params.applicationID).status }}</p>
        <p>Division: {{ applicationDivisionDisplay(application($route.params.applicationID).division) }}</p>
      </div>
