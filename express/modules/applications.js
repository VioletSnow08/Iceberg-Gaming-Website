...
router.post('/create/:division', async (req, res) => {
  ...
  } else if(division === "iceberg" && !utils.doesUserContainRolesAuth(req.user.roles, "[ICE] Member")) {
    let {
      steamURL, age, hobbies, gamesTheyJoinFor, hoursInGamesTheyJoinFor, areYouInAnyCommunities, whyJoin, whereDidYouHearUsFrom
    } = req.body;
    let createdAt = DateTime.local().setZone('America/Chicago').toFormat('yyyy-MM-dd HH:mm:ss');
